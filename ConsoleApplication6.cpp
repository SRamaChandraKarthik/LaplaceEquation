...
void display_soln_linearly(vector<double> V, int Nx)
{
	for (int i = 0; i < Nx; i++)
	{
		cout << V[i] << " ";
		...
		for (int i = 0; i < pow(Nx - 2,2); i++)
		{
			for (int j = 0; j < pow(Nx - 2, 2); j++)
			{
				if (i == j)
					...
					sol_new[i] += A[i][j] * sol_old[i];
			}
			sol_new[i] = sol_new[i] / A[i][i];
			
			//cout << sol_new[i] << endl;
			
			norm += sqrt(abs(sol_new[i] - sol_old[i]));
			sol_old[i] = sol_new[i];
			//cout << norm << endl;;
		}
	}
	display_soln_linearly(sol_new, pow(Nx - 2, 2));
	return 0;
}
